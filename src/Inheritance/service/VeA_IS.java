...
import Inheritance.model.Person;
...
public class VeA_IS {
    ...
    public static void main(String[] args) {
        ...
        // PERSONS = professors + students
        //TODO: ARRAYLIST ar students + professors
        persons.add(prof_kar);
        persons.add(prof_and);
        persons.add(prof_est);
        persons.add(prof_uld);
        persons.add(st_ev);
        persons.add(st_av);
        persons.add(st_ee);
        persons.add(st_ao);
        ...
        //izprintēt professors + master degree
        for (Person temp: persons){
            if (temp instanceof Professor){
                Professor profTemp = (Professor) temp;
                if(profTemp.getDegree().equals(Degree.Master));
            }
        }
    ...
    // 5.Create a new function which sort all students by their average grade;
    public static ArrayList<Student> sortStudents(){
        //5.1. Pārkopēt elementus no studentu saraksta:
        ArrayList<Student> students = new ArrayList<>();
        for (Student temp: studentArrayList){
            students.add(temp);
        }
        //5.2. Bubble sort algoritms
        ...
        //5.3. Atgriezt sakārtoto sarakstu
        return students;
    }
